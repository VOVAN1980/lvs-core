...
export class LvsState {
  ...
  constructor(selfId: NodeId, initialPeers: NodeId[]) {
    ...
    // создаём аккаунты для себя и известных пиров
    const all = new Set<NodeId>([selfId, ...initialPeers]);
    for (const id of all) {
      this.accounts.set(id, {
        id,
        vu: id === selfId ? 1000 : 0,   // стартовый баланс только у себя
        tc: id === selfId ? 1 : 0.5     // минимальный доверительный baseline
      });
    }
  }
  ...
  applyLocalTransfer(to: NodeId, amount: number): Transfer | null {
    ...
    if (from.vu - amount < 0) return null; // без овердрафта

    from.vu -= amount;
    toAcc.vu += amount;

    // простая логика доверия: отправитель чуть теряет, получатель чуть набирает
  ...
  applySnapshotMerge(snapshot: NodeSnapshot): { vuDelta: number; trustDelta: number } {
    // Мержим по узлам: берём среднее по vu и tc
    let vuDelta = 0;
    let trustDelta = 0;
    ...
    for (const incoming of snapshot.accounts) {
      ...
      // Weighted average с приоритетом более "богатого" по доверия
